...
public class EntityPanel extends VBox {
    private class EntityListCell
        ...
        EntityListCell() {
            ...
            Tooltip.install(m_indicator, new Tooltip("No Position component"));
            HBox.setMargin(
                m_indicator,
                new Insets(INDICATOR_TOP, INDICATOR_RIGHT, INDICATOR_BOTTOM, INDICATOR_LEFT)
            );
            m_root.setAlignment(Pos.CENTER_LEFT);
            m_root.setFillHeight(true);
            m_root.getChildren().addAll(m_nameLabel, m_spacer, m_indicator);
        }
        ...
        protected void updateItem(Integer index, boolean empty) {
            ...
            m_nameLabel.setText(index + ": " + entity.id());
            boolean hasPosition = entity.getPositionComponent() != null;
    ...
    private static final String STYLE_ENTITY_PANEL = "editor-entity-panel";
    private static final String STYLE_TOOLBAR_LABEL_BOLD = "editor-toolbar-label-bold";
    private static final String STYLE_NO_POSITION = "editor-entity-no-position";
    ...
    private static final String CSS = """
        ...
        """.formatted(STYLE_ENTITY_PANEL, STYLE_TOOLBAR_LABEL_BOLD, STYLE_NO_POSITION);
    ...
    public EntityPanel(EditorMapModel model) {
        ...
        m_entityList.setCellFactory(lv -> new EntityListCell());
        ...
        var buttonBar = new HBox(BUTTON_SPACING, addBtn, removeBtn);
        buttonBar.setAlignment(Pos.CENTER_LEFT);
        buttonBar.setPadding(new Insets(BUTTON_BAR_TOP_PADDING, 0, 0, 0));

        getChildren().addAll(titleLabel, m_entityList, buttonBar);
        refreshList();
    }
    ...
    private void onAdd() {
        EntityAddDialog.showAdd().ifPresent(result -> {
            var entity = new EditorEntity(result.entityId(), List.of());
            m_model.addEntity(entity);
            refreshList();
            fireEntitiesChanged();
            int newIndex = m_model.getEntities().size() - 1;
            m_entityList.getSelectionModel()
                .select(newIndex);
        });
    }

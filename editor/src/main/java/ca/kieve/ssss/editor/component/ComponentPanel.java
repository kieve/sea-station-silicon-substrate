...
public class ComponentPanel
    extends
    CompactTreeTable<ComponentPanel.ComponentRow> {
    enum OverrideStatus {
        NONE,
        REPLACED,
        ADDED
    ...
    record ComponentRow(
        ...
        ComponentRow(String name, String value) {
            this(name, value, OverrideStatus.NONE, null, null, null, false, null, null);
        }
    }

    @FunctionalInterface
    public interface PropertyEditCallback {
        void onPropertyEdited(String componentTypeName, String propertyName, String newValue);
    }
    ...
    private class EditableValueCell
        extends
        TreeTableCell<ComponentRow, String> {
        ...
        private void applyTypeFilter(TextField field, Object rawValue, Class<?> fieldType) {
            ...
            if (!isFloatType(rawValue, fieldType)) {
                return;
            }
            UnaryOperator<TextFormatter.Change> filter = change -> {
                String newText = change.getControlNewText();
    ...
    private static final String ROW_CSS = """
        ...
        """.formatted(STYLE_COMPONENT_PANEL, STYLE_ROW_REPLACED, STYLE_ROW_ADDED, INDICATOR_WIDTH);
    ...
    public void showEntity(String entityId) {
        ...
        if (!def.parents().isEmpty()) {
            ...
            for (String parent : def.parents()) {
                parentsItem.getChildren().add(new TreeItem<>(new ComponentRow("", parent)));
            }
    ...
    public void showMapEntity(String baseEntityId, List<ComponentDefinition> overrides) {
        commitPendingEdit();
        ...
        List<ComponentDefinition> baseComponents = def.resolveComponents(m_registry);
    ...
    public void clear() {
        ...
        emptyRoot.getChildren().add(new TreeItem<>(new ComponentRow("Nothing selected", "")));
        setRoot(null);
        setRoot(emptyRoot);
    }
    ...
    private ContextMenu buildContextMenu(ComponentRow item) {
        ...
        if (item.status() == OverrideStatus.REPLACED
            ...
            var revertItem = new MenuItem("Revert");
            revertItem.setOnAction(
                e -> m_onComponentOverride.onPropertyReverted(
                    item.componentType(),
                    item.propertyName()
                )
            );
    ...
    private TreeItem<ComponentRow> buildComponentItem(ComponentDefinition comp, boolean editable) {
    ...
    private TreeItem<ComponentRow> buildComponentItem(
        ...
        List<FieldInfo> fields = ComponentIntrospector.getEditableFields(compClass);
        boolean isEnum = compClass.isEnum();

        if (fields.isEmpty() && !isEnum) {
            return new TreeItem<>(
                new ComponentRow(name, "(marker)", status, null, name, null, false, null, null)
            );
        }

        var item = new TreeItem<>(
            new ComponentRow(name, "", status, null, name, null, false, null, null)
        );

        Map<String, Object> baseProps = baseComp != null ? baseComp.properties() : Map.of();
        ...
        if (isEnum) {
            ...
            for (var entry : props.entrySet()) {
                ...
                if (status == OverrideStatus.REPLACED
                    ...
                    String basePropValue = String.valueOf(baseProps.get(propName));
            ...
            for (FieldInfo field : fields) {
                ...
                if (status == OverrideStatus.REPLACED
                    ...
                    String basePropValue = String.valueOf(baseProps.get(field.name()));
                    if (!basePropValue.equals(propValue)) {
                        baseValue = basePropValue;
                    }
                }

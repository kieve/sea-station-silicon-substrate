...
public class ZLevelOverlay extends HBox {
    ...
    private static final String STYLE_Z_STEP_BUTTON = "editor-z-step-button";
    private static final String STYLE_Z_ADD_BUTTON = "editor-z-add-button";
    ...
    private static final String CSS = EditorTheme.OVERLAY_CSS + """
        ...
        """.formatted(STYLE_Z_STEP_BUTTON, STYLE_Z_ADD_BUTTON, OVERLAY_RADIUS);
    ...
    public ZLevelOverlay() {
        ...
        addBtn.minWidthProperty().bind(addBtn.heightProperty());
        addBtn.prefWidthProperty().bind(addBtn.heightProperty());
        addBtn.setOnAction(e -> {
            if (m_onAddLayerRequested != null) {
                m_onAddLayerRequested.run();
            }
        });
        HBox.setMargin(addBtn, new Insets(-OVERLAY_PAD_Y, -OVERLAY_PAD_X, -OVERLAY_PAD_Y, 0));

        getChildren().addAll(label, m_zCombo, stepButtons, addBtn);
    }

    public void setOnZLevelRequested(Consumer<Integer> callback) {
        m_onZLevelRequested = callback;
    }

    public void setOnAddLayerRequested(Runnable callback) {
